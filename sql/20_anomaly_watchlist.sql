...
BEGIN
  ...
  SELECT
    ...
    COALESCE(NULLIF(STDDEV_SAMP(AVG_TEMPERATURE_F), 0), 1.0) AS BL_TEMP_STD,
    AVG(AVG_POWER_W) AS BL_POWER_MEAN,
    COALESCE(NULLIF(STDDEV_SAMP(AVG_POWER_W), 0), 1.0) AS BL_POWER_STD,
    AVG(AVG_LATENCY_MS) AS BL_LAT_MEAN,
    COALESCE(NULLIF(STDDEV_SAMP(AVG_LATENCY_MS), 0), 1.0) AS BL_LAT_STD,
    AVG(AVG_PACKET_LOSS_PCT) AS BL_LOSS_MEAN,
    COALESCE(NULLIF(STDDEV_SAMP(AVG_PACKET_LOSS_PCT), 0), 0.1) AS BL_LOSS_STD,
    AVG(AVG_BRIGHTNESS) AS BL_BRIGHT_MEAN,
    COALESCE(NULLIF(STDDEV_SAMP(AVG_BRIGHTNESS), 0), 1.0) AS BL_BRIGHT_STD,
    AVG(TOTAL_ERRORS) AS BL_ERR_MEAN,
    COALESCE(NULLIF(STDDEV_SAMP(TOTAL_ERRORS), 0), 1.0) AS BL_ERR_STD
...
-- Convenience: run once (demo-stable scenario lock) and preview current watchlist
CALL OPERATIONS.REFRESH_WATCHLIST(
  'SCENARIO_LOCK',
  (SELECT DEMO_AS_OF_TS FROM OPERATIONS.V_DEMO_TIME),
  14,
  1,
  25
);
