...
-- Clean up older procedure signatures to avoid ambiguous overloading when defaults are used.
DROP PROCEDURE IF EXISTS OPERATIONS.REFRESH_FAILURE_PREDICTIONS(STRING, TIMESTAMP_NTZ, INT, FLOAT);
DROP PROCEDURE IF EXISTS OPERATIONS.REFRESH_FAILURE_PREDICTIONS(STRING, TIMESTAMP_NTZ, INT, FLOAT, INT);
...
CREATE OR REPLACE PROCEDURE OPERATIONS.REFRESH_FAILURE_PREDICTIONS(
  ...
  MIN_PROBABILITY FLOAT DEFAULT 0.35,
  MAX_PREDICTIONS INT DEFAULT 5
)
RETURNS STRING
LANGUAGE SQL
AS
...
BEGIN
  ...
  WITH w AS (
    ...
    UNION ALL
    -- Demo-only: include one "control" device as a likely false positive to demonstrate
    -- the precision/recall tradeoff at a balanced operating point.
    SELECT
      DEVICE_ID,
      SCORE_OVERALL,
      SCORE_THERMAL,
      ...
      TOP_SIGNALS
    FROM PREDICTIVE_MAINTENANCE.OPERATIONS.WATCHLIST_CURRENT
    WHERE :MODE = 'SCENARIO_LOCK' AND DEVICE_ID = '4501'
  ),
  mapped AS (
    SELECT
      DEVICE_ID,
      ...
      IFF(
        :MODE = 'SCENARIO_LOCK' AND DEVICE_ID = '4501',
        0.72,
        LEAST(0.98, GREATEST(0.05, 0.15 + (0.90 * SCORE_OVERALL)))
